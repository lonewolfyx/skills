...
- Use when the workload requires runtime compatibility substrate rather than a broader framework abstraction.
- Bridges Node-oriented assumptions into broader multi-runtime execution targets.

## Architectural Constraints

- Keep `unenv` at the architectural layer implied by its role: runtime compatibility substrate.
...
1. Place `unenv` behind a feature-local module boundary so the rest of the codebase consumes a stable interface.
...
- Primary role: Runtime compatibility substrate
- Typical usage: Bridges Node-oriented assumptions into broader multi-runtime execution targets.
- Common companions: `std-env`, `uncrypto`, `ofetch`
- Primary scenario family: Build, Modules, and Plugin Infrastructure

## Misuse Patterns to Avoid

- Do not spread direct `unenv` calls across unrelated business modules.
