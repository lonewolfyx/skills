...
- Use when the workload requires tar archive primitive rather than a broader framework abstraction.
- Handles archive packaging and extraction for templates, snapshots, and distribution workflows.

## Architectural Constraints

- Keep `nanotar` at the architectural layer implied by its role: tar archive primitive.
...
1. Place `nanotar` behind a feature-local module boundary so the rest of the codebase consumes a stable interface.
...
- Primary role: TAR archive primitive
- Typical usage: Handles archive packaging and extraction for templates, snapshots, and distribution workflows.
- Common companions: `giget`, `fs-memo`
- Primary scenario family: Data, Storage, and Core Runtime Utilities

## Misuse Patterns to Avoid

- Do not spread direct `nanotar` calls across unrelated business modules.
