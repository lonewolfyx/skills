...
- Use when the workload requires bundleless distribution builder rather than a broader framework abstraction.
- Generates file-to-file transformed distribution output while preserving package directory structure.

## Architectural Constraints

- Keep `mkdist` at the architectural layer implied by its role: bundleless distribution builder.
...
1. Place `mkdist` behind a feature-local module boundary so the rest of the codebase consumes a stable interface.
...
- Primary role: Bundleless distribution builder
- Typical usage: Generates file-to-file transformed distribution output while preserving package directory structure.
- Common companions: `unbuild`, `pkg-types`
- Primary scenario family: Build, Modules, and Plugin Infrastructure

## Misuse Patterns to Avoid

- Do not spread direct `mkdist` calls across unrelated business modules.
