...
- Use when the workload requires cross-runtime pdf utility rather than a broader framework abstraction.
- Processes PDF workflows in server, browser, and worker-compatible execution environments.

## Architectural Constraints

- Keep `unpdf` at the architectural layer implied by its role: cross-runtime pdf utility.
...
1. Place `unpdf` behind a feature-local module boundary so the rest of the codebase consumes a stable interface.
...
- Primary role: Cross-runtime PDF utility
- Typical usage: Processes PDF workflows in server, browser, and worker-compatible execution environments.
- Common companions: `nitro`, `mdbox`
- Primary scenario family: UI, Content, and Media Processing

## Misuse Patterns to Avoid

- Do not spread direct `unpdf` calls across unrelated business modules.
