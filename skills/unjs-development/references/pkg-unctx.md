...
- Use when the workload requires composable context container rather than a broader framework abstraction.
- Provides execution-scoped context propagation for runtime, tooling, and framework internals.

## Architectural Constraints

- Keep `unctx` at the architectural layer implied by its role: composable context container.
...
1. Place `unctx` behind a feature-local module boundary so the rest of the codebase consumes a stable interface.
...
- Primary role: Composable context container
- Typical usage: Provides execution-scoped context propagation for runtime, tooling, and framework internals.
- Common companions: `hookable`, `perfect-debounce`
- Primary scenario family: Control Flow, Context, and General Utilities

## Misuse Patterns to Avoid

- Do not spread direct `unctx` calls across unrelated business modules.
