...
- Use when the workload requires url utility toolkit rather than a broader framework abstraction.
- Handles URL joining, normalization, encoding, decoding, and query manipulation.

## Architectural Constraints

- Keep `ufo` at the architectural layer implied by its role: url utility toolkit.
...
1. Place `ufo` behind a feature-local module boundary so the rest of the codebase consumes a stable interface.
...
- Primary role: URL utility toolkit
- Typical usage: Handles URL joining, normalization, encoding, decoding, and query manipulation.
- Common companions: `ofetch`, `h3`, `pathe`
- Primary scenario family: Control Flow, Context, and General Utilities

## Misuse Patterns to Avoid

- Do not spread direct `ufo` calls across unrelated business modules.
