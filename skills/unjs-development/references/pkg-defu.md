...
- Use when the workload requires deterministic deep-default merge rather than a broader framework abstraction.
- Performs deep merge operations optimized for default-value inheritance and configuration normalization.

## Architectural Constraints

- Keep `defu` at the architectural layer implied by its role: deterministic deep-default merge.
...
1. Place `defu` behind a feature-local module boundary so the rest of the codebase consumes a stable interface.
...
- Primary role: Deterministic deep-default merge
- Typical usage: Performs deep merge operations optimized for default-value inheritance and configuration normalization.
- Common companions: `c12`, `rc9`, `untyped`
- Primary scenario family: Configuration, Schemas, and Project Metadata

## Misuse Patterns to Avoid

- Do not spread direct `defu` calls across unrelated business modules.
